import { loadStore, parseArgv, saveStore } from "./store.mjs";
...
export async function sendCode(phone, gid, aid, pid) {
...
export async function smsLogin(phone, code, smsId, gid, aid, pid) {
...
export function extractSmsId(payload) {
  const data = payload?.data ?? payload?.body ?? payload?.result ?? payload;
  if (!data || typeof data !== "object") return null;
  const smsId = data.smsId ?? data.smsid ?? data.sms_id ?? data.id;
  return smsId ?? null;
}
